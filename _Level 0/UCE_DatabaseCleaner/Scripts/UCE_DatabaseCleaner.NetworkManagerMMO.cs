...
public partial class NetworkManagerMMO
{
...
#elif _SQLITE
                            var checkAccount = Database.singleton.connection.FindWithQuery<accounts>("SELECT banned FROM accounts WHERE name=?", accountName);
    ...
    public void UCE_DatabaseCleanup(string accountName)
    {
        ...
        foreach (string accountChar in accountChars)
        {
            Database.singleton.connection.Execute("DELETE FROM character_buffs WHERE character=?", accountChar);
            Database.singleton.connection.Execute("DELETE FROM character_inventory WHERE character=?", accountChar);
            Database.singleton.connection.Execute("DELETE FROM character_equipment WHERE character=?", accountChar);
            Database.singleton.connection.Execute("DELETE FROM character_skills WHERE character=?", accountChar);
            Database.singleton.connection.Execute("DELETE FROM character_quests WHERE character=?", accountChar);
            Database.singleton.connection.Execute("DELETE FROM character_orders WHERE character=?", accountChar);

            foreach (string charTable in DatabaseCleaner.characterTables)
            {
                if (charTable != "")
                {
                    var compare = Database.singleton.connection.FindWithQuery<sqlite_master>("SELECT count(*) FROM sqlite_master WHERE type='table' AND name=?", charTable);
                    if (0 < compare.count)
                        Database.singleton.connection.Execute("DELETE FROM " + charTable + " WHERE character=?", accountChar);
                }
            }
            ...
            foreach (string accountTable in DatabaseCleaner.accountTables)
            {
                if (accountTable != "")
                {
                    var compare = Database.singleton.connection.FindWithQuery<sqlite_master>("SELECT count(*) FROM sqlite_master WHERE type='table' AND name=?", accountTable);
                    if (0 < compare.count)
                        Database.singleton.connection.Execute("DELETE FROM " + accountTable + " WHERE account=?", accountName);
                }
            }
        }

        Database.singleton.connection.Execute("DELETE FROM characters WHERE account=?", accountName);
        Database.singleton.connection.Execute("DELETE FROM accounts WHERE name=?", accountName);
#endif
